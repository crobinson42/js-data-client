...
function Record(props, opts) {
  ...
  _set(
    ...
    opts.keepChangeHistory === undefined
      ? mapper
        ? mapper.keepChangeHistory
        : true
      : opts.keepChangeHistory,
  )
...
export default Component.extend(
  {
    ...
    changes(opts) {
      opts || (opts = {})
      return utils.diffObjects(
        typeof this.toJSON === 'function' ? this.toJSON(opts) : this,
        this._get('previous'),
        opts,
      )
    },
    ...
    destroy(opts) {
      ...
      return superMethod(mapper, 'destroy')(
        utils.get(this, mapper.idAttribute),
        opts,
      )
    },
    ...
    removeInverseRelation(currentParent, id, inverseDef, idAttribute) {
      if (inverseDef.type === hasOneType) {
        ...
        if (id === undefined) {
          ...
          utils.remove(
            children,
            child => child === this || id === utils.get(child, idAttribute),
          )
        }
      }
    },
