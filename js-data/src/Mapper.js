  ...
  return function(...args) {
    ...
    if (validatingHooks.indexOf(op) !== -1 && !opts.noValidate) {
      ...
      const errors = this.validate(
        args[op === "beforeUpdate" ? 1 : 0],
        utils.pick(opts, ["existingOnly"])
      );
...
function Mapper(opts) {
  ...
  if (this.recordClass) {
    ...
    if (
      Record.prototype.isPrototypeOf(
        Object.create(this.recordClass.prototype)
      ) &&
      this.schema &&
      this.schema.apply &&
      this.applySchema
...
export default Component.extend({
  ...
  _commitChanges(recordOrRecords, newValues) {
    if (utils.isArray(recordOrRecords)) {
      return recordOrRecords.map((record, i) =>
        this._commitChanges(record, newValues[i])
      );
    }
  ...
  _createOrAssignChildRecordIfRequired(props, context) {
    ...
    return utils.Promise.all(tasks).then(() => props);
  },
      ...
      .then(records => {
        ...
        utils.forEachRelation(this, opts, (def, optsCopy) => {
          ...
          if (
            ...
            relationData.length === records.length
          ) {
        ...
        return utils.Promise.all(tasks)
          ...
          .then(() => {
            const createdRecordsData = opts.raw
            ...
            utils.forEachRelation(this, opts, (def, optsCopy) => {
              ...
              if (def.type === hasManyType) {
                ...
                this.log(
                  "warn",
                  "deep createMany of hasMany type not supported!"
                );
  ...
  createRecord(props, opts) {
    ...
    if (!utils.isObject(props)) {
      throw utils.err(`${DOMAIN}#createRecord`, "props")(
        400,
        "array or object",
        props
      );
    ...
    return !RecordCtor || props instanceof RecordCtor
      ? props
      : new RecordCtor(props, opts);
  },
      ...
      .then(_value => {
        if (args[config.beforeAssign] !== undefined) {
          ...
          args[config.beforeAssign] =
            _value === undefined ? args[config.beforeAssign] : _value;
        }
  ...
  _runHook(hookName, ...hookArgs) {
    const defaultValueIndex =
      hookName.indexOf("after") === 0 ? hookArgs.length - 1 : 0;

    return utils
      .resolve(this[hookName](...hookArgs))
      .then(
        overridenResult =>
          overridenResult === undefined
            ? hookArgs[defaultValueIndex]
            : overridenResult
      );
  },

  _invokeAdapterMethod(method, propsOrRecords, opts) {
    console.log("****method, propsOrRecords, opts");
    console.log(method, propsOrRecords, opts);
    ...
    if (utils.isArray(propsOrRecords)) {
      object = propsOrRecords.map(record =>
        this.toJSON(record, conversionOptions)
      );
    } else {
      ...
      utils.forEachRelation(this, opts, (def, optsCopy) => {
        ...
        if (relationData) {
          ...
          if (utils.isArray(relationData)) {
            ...
            def.setLocalField(
              json,
              def.getRelation().toJSON(relationData, optsCopy)
            );
          }
        }
      });
  ...
  validate(record, opts) {
    ...
    if (utils.isArray(record)) {
      const errors = record.map(_record =>
        schema.validate(_record, utils.pick(_opts, ["existingOnly"]))
      );
      ...
      utils.forOwn(group, (relations, _name) => {
        ...
        relations.forEach(def => {
          ...
          if (typeof Relation[type] !== "function") {
            throw utils.err(DOMAIN, "defineRelations")(
              400,
              "relation type (hasOne, hasMany, etc)",
              type,
              true
            );
          }
